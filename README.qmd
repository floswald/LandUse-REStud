...
1. Navigate to (hidden) folder `/.devcontainer`
...
# Possible Issues with Devcontainer

## ENV var not resolving

We have had reports from users where the variable resolution from host shell into the container's environment did not work. This is related to object `source=${localEnv:STATA_LICENSE_PATH}` on line 10 of `./devcontainer/devcontainer.json`. A working solution is to just hardcode the path to the stata license file, i.e. I would change line 10 to

```bash
"source=/Users/floswald/Dropbox/licenses/stata.lic,target=/usr/local/stata/stata.lic,type=bind"
```

## Permissions of folder to be mounted

All directories to be mounted need to have sufficient permissions (read,write,execute) for the current user. Some users have reported problems from storing the stata license like I did in their dropbox folder, for instance. Consider moving this to a location with sufficient access permissions.
